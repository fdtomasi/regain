function [cv_output,model] = grl_kcv(X,Y,varargin)
%GRL_KCV Parameters choice through cross validation for the group lasso algorithm
%(variable selector) followed by Regularized Least Squares (for debiasing).
%   CV_OUTPUT = GRL_KCV(X,Y) Given training set, (X,Y), performs leave-one-out cross
%   validation for GRL algorithm. GRL is used for selection of the
...
%   [CV_OUTPUT,MODEL] = GRL_KCV(X,Y) Also returns the estimated model
%
%   GRL_KCV(...,'PropertyName',PropertyValue,...) sets properties to the
...
%        of the model learned via GRL without RLS-debiasing; if
%        'two_steps' computes error of the model learned via GRL with
...
%        cross-validation error for the 1-step framework (GRL only)
...
%        cross-validation error for the 2-steps framework (GRL and RLS)
...
%            Y = X*beta_true;
%            Y = awgn(Y,10);
%            blocks = mat2cell(reshape(1:1000,5,200),5,ones(200,1));
%            [cv_output,model] = grl_kcv(X,Y,'blocks',blocks);
% 
...
%            Y = awgn(Y,10);
%            [cv_output,model] = grl_kcv(X,Y,,'blocks',blocks,...
%               'protocol','one_step','K',5,'smooth_par',0.1);
% 
...
%              'plot',true);
%
%   See also GRL_REGPATH, RLS_REGPATH, GRL_LEARN
%
...
for i = 1:length(sets);
    ...
    [selected{i},tmin,n_iter(:,i),beta] = grl_regpath(Xtr,Ytr,blocks,tau_values,'weights',weights,'smooth_par',smooth_par);
    ...
    for t = tmin:ntau;
        if woRLS
            ...
            pred = grl_pred(model,Xts,Yts,err_type);
            ...
            pred = grl_pred(model,Xtr,Ytr,err_type);
        ...
        if withRLS
            ...
            for j = 1:length(lambda_values);
                ...
                % evaluates prediction error on validation set
                pred = grl_pred(model,Xts(:,selected{i}(:,t)),Yts,err_type);
                err_KCV2(i,t,j) = pred.err;
                % when evaluating training error, offset must not be added
                % because training data is already centered
                model.offset = 0;
                % evaluates prediction error on training set
                pred = grl_pred(model,Xtr(:,selected{i}(:,t)),Ytr,err_type);
...
if nargout>1;
    if woRLS;
        [model.beta_1step,model.offset_1step] = grl_learn(X,Y,blocks,tau_values(t_opt:end),'weights',weights,'smooth_par',smooth_par);
        model.selected_1step = model.beta_1step~=0;
    end
    if withRLS;
        [model.beta_2steps,model.offset_2steps] = grl_learn(X,Y,blocks,tau_values(t_opt2:end),'weights',weights,'smooth_par',smooth_par,'RLS_par',lambda_opt(t_opt2));
...
if plotting;
    if smooth_par==0;
        ...
        figure('Name','GRL regularization')
    end
    c = 0;
    if woRLS
        c = c+1;
